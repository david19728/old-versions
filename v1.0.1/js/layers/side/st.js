    ...
    infoboxes: {
        1: {
            ...
            body() {
                ...
                if(hasMilestone("b",0))
                {
                    t1display += "维度献祭将你的序数维度O<sub>8</sub>乘以" + player.o.sacMult + "×<br>"
                }
                ...
                {
                    t1display += "如果你每秒写一个数字，那么写下你的序数点数量（不用科学计数法）需要" + formatTime(player.points.logBase(10)) + "。"
                }
                return t1display
            },
        },
        2: {
            ...
            body() {
                ...
                {
                    t2display += "，将你的所有维度乘以" + ExpantaNum(5).pow(player.p.points) + "×<br>"
                }
                t2display += "你的最佳声望点是" + player.p.best + "<br>"
                t2display += "距离上一次声望已经过去了" + formatTime(player.p.resetTime) + "<br>"
                t2display += "大约需要" + formatTime(ExpantaNum(player.p.resetTime).div(player.points.logBase(ExpantaNum(3).pow(30).mul(ExpantaNum(3).pow(75).pow(player.p.points.add(1)))))) + "才能进行下一次声望。（还剩" + formatTime(ExpantaNum(player.p.resetTime).div(player.points.logBase(ExpantaNum(3).pow(30).mul(ExpantaNum(3).pow(75).pow(player.p.points.add(1))))).sub(player.p.resetTime)) + "）<br>"
                if(hasUpgrade("p",11))
                {
                    t2display += "“首领之力”升级将你的O<sub>1~7</sub>生产变为" + getBuyableAmount("o",11).add(1).pow(3) + "倍<br>"
                }
                if(hasUpgrade("p",13))
                {
                    t2display += "“BP协同”升级将你的声望底数加上+" + player.b.dist.add(1).div(5) + "<br>"
                }
                ...
                t2display += "你的最佳助推器是" + player.b.best + "<br>"
                t2display += "距离上一次维度提升已经过去了" + formatTime(player.b.resetTime) + "<br>"
                t2display += "大约需要" + formatTime(ExpantaNum(player.b.resetTime).div(player.points.logBase(ExpantaNum(3).pow(30).mul(ExpantaNum(3).pow(75).pow(player.b.points.add(1)))))) + "才能进行下一次维度提升。（还剩" + formatTime(ExpantaNum(player.b.resetTime).div(player.points.logBase(ExpantaNum(3).pow(30).mul(ExpantaNum(3).pow(75).pow(player.b.points.add(1))))).sub(player.b.resetTime)) + "）<br>"
                if(hasUpgrade("b",11))
                {
                    t2display += "“OB连击”升级将你的助推器底数加上+" + getBuyableAmount("o",11).add(getBuyableAmount("o",12)).add(getBuyableAmount("o",13)).add(getBuyableAmount("o",14)).add(getBuyableAmount("o",15)).add(getBuyableAmount("o",16)).add(getBuyableAmount("o",17)).add(getBuyableAmount("o",18)).sqrt().div(10) + "<br>"
                }
                if(hasUpgrade("b",13))
                {
                    t2display += "“PB协同”升级将你的助推器底数加上+" + player.p.best.mul(0.5)
                }
