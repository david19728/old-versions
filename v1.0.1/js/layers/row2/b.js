addLayer("b", {
    ...
    effectDescription()
    {
        ...
        boosterDisplay += player.b.bstBase.pow(player.b.points).sub(1).toFixed(6)
    ...
    update(diff){
        if(hasUpgrade("b",11))
        {
            player.b.bstBase = ExpantaNum(5).add(getBuyableAmount("o",11).add(getBuyableAmount("o",12)).add(getBuyableAmount("o",13)).add(getBuyableAmount("o",14)).add(getBuyableAmount("o",15)).add(getBuyableAmount("o",16)).add(getBuyableAmount("o",17)).add(getBuyableAmount("o",18)).sqrt().div(10))
        }
        else
        {
            player.b.bstBase = ExpantaNum(5)
        }
        if(hasUpgrade("b",13))
        {
            player.b.bstBase = player.b.bstBase.add(player.p.best.mul(0.5))
        }
    ...
    upgrades: {
        11: {
            title: "OB连击",
            description: "总维度购买数量加成助推器底数",
            cost: new ExpantaNum(3),
            effectDisplay()
            {
                return "+" + getBuyableAmount("o",11).add(getBuyableAmount("o",12)).add(getBuyableAmount("o",13)).add(getBuyableAmount("o",14)).add(getBuyableAmount("o",15)).add(getBuyableAmount("o",16)).add(getBuyableAmount("o",17)).add(getBuyableAmount("o",18)).sqrt().div(10).toFixed(6)
            }
        },
        12: {
            title: "神之微笑",
            description: "献祭指数+0.05",
            cost: new ExpantaNum(5)
        },
        13: {
            title: "PB协同",
            description: "最高声望加成助推器底数",
            cost: new ExpantaNum(8),
            effectDisplay()
            {
                return "+" + player.p.best.mul(0.5)
            }
        },
        14: {
            title: "升维",
